      ...
      this.homeService.getPlatformSubscriptions()
        .then((jresponse: any) => {
          if (jresponse) {
            ...
            let starterPlanObject = this.subscriptions.find((data) => data.planAmount == 2900 && data.status == true);

            let growthPlanObject = this.subscriptions.find((data) => data.planAmount == 9900 && data.status == true);
