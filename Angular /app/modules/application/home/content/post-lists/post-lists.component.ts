 ...
 comment;
  ...
  async ngOnInit() {
    ...
    if (this.activatedRoute.snapshot.queryParams.userId) {
      ...
      this.homeWsId = this.homeService.activityWsId;
      if (this.homeOrgId === "") {
      ...
      this.appViewService
        ...
        .then((jresponse: JReponse) => {
          if (jresponse.success && !_.isEmpty(jresponse.body)) {
            ...
            //       return item;
            //     } else {
            //      // activity.subActivityTotal = activity.subActivityTotal > 0 ? activity.subActivityTotal - 1 : 0;
            //       return false;
            //     }
            //   })
            //   arr = _.compact(arr);
  ...
  submitForm() {
    ...
    // if (_.isEmpty(this.comment) && lengthCheck === 0) {
  ...
  async findUrl(e) {
    ...
    if (!_.isEmpty(this.comment)) {
      let matches = this.comment.match(/\bhttps?:\/\/\S+/gi);
  ...
  addCommentRecord(event, type?, activityData?) {
    let activity;
    let comment;
  ...
  editCommentRecord(event, activityData, currentComment, type?) {
  ...
  addSubComment(commentObj, activity, type?, event?, parentComment?) {
    let commentText = event.target.innerText;
    selectedMentionUsers.forEach((element) => {
      commentText = commentText.replace(element.name, element.id);
    });
